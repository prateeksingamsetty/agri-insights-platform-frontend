...
const OperatingCosts = () => {
  ...
  const checkProductionDetailsPresent = async (): Promise<void> => {
    ...
    try {
      if (loggedIn && email != null) {
        ...
        const storedProductiondetailsInputs =
          localStorage.getItem('productionInputs')
  ...
  const fetchOperatingCosts = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/operating-costs/operatingCostoutputDetails/${email}`
      )
  ...
  const handleSubmitOperatingCost = async (
    ...
    try {
      ...
      const transformedInputs = {
        ...
        inseminationConventionalFees: Number(
          userInputs.inseminationConventionalFees
        ),
        inseminationConventionalBeefFees: Number(
          userInputs.inseminationConventionalBeefFees
        ),
        ...
        additionalManagementCostsPercentage: Number(
          userInputs.additionalManagementCostsPercentage
        ),
      ...
      if (loggedIn && email != null) {
        response = await axios.patch(
          `${BASE_URL}/operating-costs/updateInput/${email}`,
          transformedInputs
        )
      } else {
        const storedProductiondetailsInputs =
        ...
        if (
          storedProductiondetailsInputs !== null &&
          storedReciptsInputs !== null
        ) {
          try {
            ...
            if (labourInputs !== null) {
              labourTransformedInputs = {
                ...
                numberOfHerdHealthEmployees: Number(
                  labourInputs.numberOfHerdHealthEmployees
                ),
                herdHealthEmployeeTime: Number(
                  labourInputs.herdHealthEmployeeTime
                ),
                herdHealthEmployeeWage: Number(
                  labourInputs.herdHealthEmployeeWage
                ),
                herdHealthEmployeeBenefits: Number(
                  labourInputs.herdHealthEmployeeBenefits
                ),
                numberOfFeederEmployees: Number(
                  labourInputs.numberOfFeederEmployees
                ),
                ...
                feederEmployeeBenefits: Number(
                  labourInputs.feederEmployeeBenefits
                ),
                numberOfCropEmployees: Number(
                  labourInputs.numberOfCropEmployees
                ),
                ...
                numberOfMilkerEmployees: Number(
                  labourInputs.numberOfMilkerEmployees
                ),
                ...
                otherEmployeeBenefits: Number(
                  labourInputs.otherEmployeeBenefits
                )
              }
            } else {
        ...
        response = await axios.post(
          `${BASE_URL}/operating-costs/calculateOperatingCost`,
          {
  ...
  const textFields = [
    ...
    {
      label: 'Raised Feed Cost($)',
      value: operatingCosts.totalRaisedForageCost
    },
    {
      label: 'Purchased Feed Cost($)',
      value: operatingCosts.totalPurchasedFeedCost
    },
    {
      label: 'Dairy Operating Cost($)',
      value: operatingCosts.dairyOperatingCosts
    },
